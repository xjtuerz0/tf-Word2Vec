...
with graph.as_default():
	# input
	tf_train_dataset = tf.compat.v1.placeholder(tf.int32, shape=(batch_size, 2*half_window_size))
	tf_train_labels = tf.compat.v1.placeholder(tf.int32, shape=(batch_size, 1))
	tf_valid_dataset = tf.constant(valid_examples, dtype=tf.int32)
	# Variables.
	embeddings = tf.Variable(tf.random.uniform(shape=(vocabulary_size, embedding_size), minval=-1.0, maxval=1.0))
	softmax_weights = tf.Variable(tf.random.truncated_normal(shape=(vocabulary_size, embedding_size), stddev=1.0 / math.sqrt(embedding_size)))
